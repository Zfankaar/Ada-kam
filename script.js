...
function pickMethod(m) {
  ...
  var map = { easypaisa: 0, jazzcash: 1, bank: 2 };
...
function setUser() {
  var data = JSON.parse(localStorage.getItem('user'));
  if (data) {
    var displayName = data.email.split('@')[0];
    document.getElementById('profileName').textContent = displayName;
    document.getElementById('profileEmail').textContent = data.email;
    document.getElementById('avatarLetter').textContent = data.email.charAt(0).toUpperCase();
...
function handleRefer() {
  ...
  if (!code) { finishSignup(); return; }

  if (VALID_CODES[code]) {
    var bonus = 0.50;
    msg.textContent = 'code accepted! +$' + bonus.toFixed(2) + ' bonus';
    msg.style.color = '#2c3a33';
    balance += bonus;
    updateDisplay();
    setTimeout(function () { finishSignup(); }, 800);
  } else {
    msg.textContent = 'invalid code, you can skip';
